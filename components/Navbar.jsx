          ...
          <ul style={{ color: "#1f2937" }} className="hidden md:flex">
            {navItems.map((item, index) => (
              <li key={index} className="ml-10 text-sm uppercase">
                <Link legacyBehavior href={navLinks[index]}>{item}</Link>
              </li>
            ))}
          </ul>
          ...
          <div className="overflow-y-auto flex flex-col fixed left-0 top-0 w-[75%] sm:w-[60%] md:w-[45%] h-screen bg-[#ecf0f3] px-8 pb-3 ease-in duration-200">
            ...
            <div className="py-4 h-full flex flex-col justify-between">
              ...
              <ul className="uppercase">
                {navItems.map((item, index) => (
                  <Link legacyBehavior key={index} href={navLinks[index]}>
              ...
              <div className="pt-10">
                <p className="tracking-widest text-primary/50">Let's get to know each other</p>
                {/* Mobile Social Links */}
                <div className="flex items-center justify-between my-4 w-full sm:w-[80%]">
                  {socialLinksMobile.map((link, index) => (
                    <a key={index} href={link.href} target="_blank" rel="noreferrer">
                      <div className="rounded-full bg-primary/10 p-3 cursor-pointer hover:scale-105 ease-in duration-300">
                        {link.icon}
                      </div>
                    </a>
                  ))}
